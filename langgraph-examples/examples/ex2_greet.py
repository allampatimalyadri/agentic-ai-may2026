# Greet example using langgraph with custom state 
...
def greet_node(state: SimpleState):
    print("\n====================1. GREET NODE ==================")
    print("Received State:", state) #current state is printed to the console for debugging 
    
    greeting_message = f"Hello {state['user_name']}! Welcome to the langgraph example."
    return {"message": greeting_message} #updated state with the greeting message

def status_node(state: SimpleState):
    print("\n====================2. STATUS NODE ==================")
    print("Received State:", state) #current state is printed to the console for debugging
    
    status_message = f" Workflow Executed." 
