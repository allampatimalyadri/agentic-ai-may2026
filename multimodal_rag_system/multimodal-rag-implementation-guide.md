...
A text-only RAG system would miss the information locked inside tables and images. A **Multimodal RAG** system parses all three content types, stores them in a shared vector database, and sends the relevant content (including actual image pixels) to a vision-capable LLM like OpenAI's GPT-4o to generate the answer.

> **A note on embeddings vs. generation.** OpenAI's embedding models are **text-only** — they turn text into vectors but cannot embed an image's pixels. So at *ingestion* time we ask a vision model to write a detailed text description of each image, and we embed that description. At *answer* time we still send the real image pixels to the vision LLM, so it reasons over the actual chart, not just its description. (Gemini offers a single multimodal embedding model that can embed pixels directly; with OpenAI we split the job in two. The end result — finding and answering questions about images — is the same.)
...
└──────────────┬──────────────┘
               ...
               ▼
┌─────────────────────────────┐
│  Query Service              │  → Retrieves top-k chunks, builds the LLM
...
| Embeddings       | OpenAI `text-embedding-3-small`       | 1536-dim, high quality, batches automatically  |
| Vector Store     | PostgreSQL + pgvector                 | Production-grade, supports HNSW indexing       |
| LLM              | OpenAI `gpt-4o-mini`                   | Multimodal — reads both text and images        |
...
3. Copy the key (starts with `sk-...`) — you will put it in `.env`. You only see it once.
...
dependencies = [
    ...
    "langchain-openai>=1.2.2",     # OpenAI embeddings + chat via LangChain
...
# Embedding model — text-embedding-3-small returns 1536 dimensions,
# which matches the VECTOR(1536) column in schema.sql
OPENAI_EMBEDDING_MODEL=text-embedding-3-small

# Chat model — must support vision (read images). gpt-4o-mini is a good,
# low-cost default. This same model is used both to describe images during
# ingestion and to answer questions at query time.
...
def _describe_image_with_openai(img_b64: str) -> str:
    """Use an OpenAI vision model to write a detailed description of an image.

    Why this exists: OpenAI's embedding models are TEXT-ONLY — they cannot turn
    an image's pixels into a vector. So for each image we generate a rich text
    description here, and THAT text is what gets embedded and made searchable.
    A good description ("bar chart of quarterly revenue rising from 120 to 180
    crore...") retrieves far better than a sparse caption like "Figure 3".

    Returns "" on any error so the caller can fall back to the caption.
    """
...
def parse_document(file_path: str) -> list[dict]:
    ...
    for item in doc.iterate_items():
        ...
        # Images are stored as base64-encoded PNG strings in metadata (and later
        # written to disk by db.store_chunks()). The "content" field holds the
        # TEXT that gets embedded — and because OpenAI embeddings cannot read
        # pixels, we generate that text with a vision model here.
        #
        # If we have the image bytes, ask the vision model for a rich
        # description. If that fails (or there are no bytes), fall back to the
        # Docling caption, then to a placeholder.
            ...
            # Generate the searchable text for this image.
            if img_b64:
...
def run_ingestion(file_path: str) -> dict:
    ...
    #   - Embeds every chunk's text with OpenAI text-embedding-3-small
...
`OpenAIEmbeddings(...)` creates an HTTP client internally. If we call it inside `store_chunks()` and `similarity_search()`, a new client is created on every function call — wasteful. Creating it once at module import time (`_embeddings = ...`) means it is created once and reused.
...
# text-embedding-3-small returns 1536-dim vectors → matches VECTOR(1536) in the
# schema. OpenAIEmbeddings batches the API requests internally, so we can hand it
# the whole list of texts at once. NOTE: these embeddings are TEXT-ONLY — image
# chunks are embedded from the vision-generated description (see docling_parser.py).
...
def _embed_texts(texts: list[str]) -> list[list[float]]:
    """Embed a list of strings; returns one 1536-dim vector per input string."""
...
def store_chunks(chunks: list[dict], doc_id: str) -> int:
    ...
    - Embeds every chunk's text (image chunks use their vision description)
    ...
    # ── Embed every chunk ─────────────────────────────────────────────────────
    # OpenAI embeddings are text-only, so text, table, AND image chunks are all
    # embedded from their `content` string. For image chunks `content` is the
    # vision-generated description, so they stay searchable by text queries.
    # OpenAIEmbeddings handles request batching internally.
    all_embeddings = _embed_texts([c["content"] for c in chunks])
...
3. Calls the OpenAI LLM with the prompt
4. Returns the answer and source metadata

### Why `SystemMessage` vs `HumanMessage`?

Chat models support different message roles:
...
# Creating a new ChatOpenAI() per request would open a new HTTP client
# each time, adding unnecessary latency. The model must be vision-capable
# (e.g. gpt-4o-mini) because the prompt can include images.
_llm = ChatOpenAI(
...
def query_documents(query: str, k: int = 5, chunk_type: str | None = None) -> dict:
    ...
    # OpenAI's multimodal chat API accepts a list of "content parts".
    ...
    # This ordering matters — the model reads content parts sequentially.
    ...
    for chunk in chunks:
        ...
        if ctype in ("text", "table"):
            ...
            # The model reads content parts in sequence; mixing text and images
            ...
            # The OpenAI vision model reads the base64 PNG bytes directly.
            message_parts.append({
                "type": "image_url",
                "image_url": {
    ...
    # response.content is usually a plain string, but some reasoning models
    # return a list of content parts instead.
...
You should see real values (not placeholders) for `OPENAI_API_KEY` and `PG_CONNECTION_STRING`.
...
query_documents()        ← query_service.py
  ...
  Step 1: similarity_search(query, k=5)     ← db.py
    _embed_texts([query]) → 1536-dim vector
...
The `OPENAI_API_KEY` in your `.env` is incorrect, expired, or still set to placeholder text. Create a fresh key at [platform.openai.com/api-keys](https://platform.openai.com/api-keys).
