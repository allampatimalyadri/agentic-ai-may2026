...
def _build_messages(
    ...
    for chunk in chunks:
        ...
        if chunk_type == "image" and chunk.get("image_base64"):
...
def query_documents(query: str, k: int = 5, chunk_type: str | None = None) -> dict:
    ...
    Returns:
        {
          "answer":  str — the model's answer grounded in the retrieved context,
...
async def stream_query_documents(
    ...
    """Async generator that streams the model's answer as SSE-formatted events.
